...
file_list_column = [
  [
    sg.Frame('Image 1 Information', [
      [ 
        sg.Text("Image Size : "),
        sg.Text(size = (13, 1), key="ImgSize")
      ], 
      [ 
        sg.Text("Color Depth : "),
        sg.Text(size=(13, 1), key="ImgColorDepth")
      ], 
    ], font = ("OpenSans", 10))
  ],
  [
    sg.Frame('Choose image 1', [
      [
  ...
  [
    sg.Frame('Image 2 Information', [
      [ 
        sg.Text("Image Size : "),
        sg.Text(size = (13, 1), key="ImgSize1")
      ], 
      [ 
        sg.Text("Color Depth : "),
  ...
  [
    sg.Frame('Choose image 2', [
      [
        sg.In(size=(19, 1), enable_events=True, key="ImgFolder1",),
        sg.FolderBrowse(),
      ],
      [
        sg.Listbox(
  ...
  [
    sg.Frame('Image Processing', [
      [
        sg.Frame("Rotate", [
          [
            sg.Slider(range=(0,3), default_value=0, size=(21, 10), orientation='horizontal', key="RotateConst", enable_events=True, visible = True)
          ],
        ], font = ("OpenSans", 10)),
      ],
      [
        sg.Frame("Flip", [
          [
            ...
            sg.Button("x,y", size=(6, 1), key="ImgFlipHorizontalVertikal"),
          ],
        ], font = ("OpenSans", 10)),
      ],
      [
        sg.Frame("Translation", [
          [
            sg.Button("Translation", size=(22, 1), key="ImgTranslation"),
          ],
          [
            sg.Text("x:", visible = True),
            sg.In(size=(8, 1), enable_events=True, key="XTrans", visible = True,  default_text=0),
            sg.Text("y:", visible = True,),
            sg.In(size=(9, 1), enable_events=True, key="YTrans", visible = True,  default_text=0),
          ],
        ], font = ("OpenSans", 10)),
      ],
      [
        sg.Frame("Zoom", [
          ...
          [
            sg.Slider(range=(1,3), default_value=1, size=(21, 10), orientation='horizontal', key="ZoomValue", enable_events=True, visible = True)
          ]
        ], font = ("OpenSans", 10)),
      ],
      [
        sg.Frame("Blending", [
          ...
          [
            ...
            sg.In(size=(7, 1), enable_events=True, key="alpha1", visible = True, default_text="1"),
            sg.Text("a2:", visible = True,),
            sg.In(size=(8, 1), enable_events=True, key="alpha2", visible = True, default_text="1"),
          ],
          [
            sg.Text("x:", visible = True),
            sg.In(size=(8, 1), enable_events=True, key="x1", visible = True, default_text="0"),
            sg.Text("y:", visible = True,),
            sg.In(size=(9, 1), enable_events=True, key="y1", visible = True, default_text="0"),
          ],
          [
...
while True:
  ...
  # Folder name was filled in, make a list of files in the folder
    ...
    try:
      filename = os.path.join(
      values["ImgFolder1"], values["ImgList1"][0]
      ) 
      ...
      #Color depth
      mode_to_coldepth1 = {"1": 1, "L": 8, "P": 8, "RGB": 24, "RGBA": 32, "CMYK": 32, "YCbCr": 24, "LAB":
      24, "HSV": 24, "I": 32, "F": 32}
  ...
  # button in basic image processing
    ...
    try:
      if(values["RotateConst"] == 0):
        deg = 0
        window["ImgProcessingType"].update("Image Rotate " + str(deg) + "deg, clockwise")
    ...
    try:
      ...
      window["ImgProcessingType"].update("Image Brightness +" + str(value_bright)) if value_bright > 0 else window["ImgProcessingType"].update("Image Brightness " + str(value_bright))
    ...
    try:
      ...
      window["ImgProcessingType"].update("Image Brightness x "+values["MulConst"])
    ...
    try:
      ...
      window["ImgProcessingType"].update("Image Brightness / "+values["DivConst"])
    ...
    try:
      ...
      window["ImgProcessingType"].update("Image Threshold " + str(value_threshold))
    ...
    try:
      ...
      window["ImgProcessingType"].update("Gamma Correction +" + str(int(constant))) if constant > 0 else window["ImgProcessingType"].update("Gamma Correction " + str(int(constant)))
    ...
    # try:
      ...
      img_output=ImgBlend(img_input, coldepth, img_input1, coldepth2, alpha1, alpha2, x1, y1)
