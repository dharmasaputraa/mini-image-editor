            ...
            if direction == "C":
                ...
                if deg == 180:
                    r, g, b = img_input.getpixel(
                        (img_input.size[1] - i - 1, img_output.size[0] - j - 1)
                    )
                if deg == 270:
...
def merge_flipped_images(img_original, coldepth):
    ...
    img_output.paste(
        img_horizontal_vertical, (img_original.size[0], img_original.size[1])
    )
...
def x_simbol_negative(img_input, coldepth):
    ...
    center_x = (
        img_output.size[0] // 2
    )  # posisi x tengah berada pada 1/4 dari lebar gambar
    center_y = (
        img_output.size[1] // 2
    )  # posisi y tengah berada pada setengah tinggi gambar
    ...
    for i in range(img_output.size[0]):
        for j in range(img_output.size[1]):
            if (
                j >= center_y
                and i
                <= center_x + ((j - center_y) * (img_output.size[0] // 2) // center_y)
                and i
                >= center_x - ((j - center_y) * (img_output.size[0] // 2) // center_y)
            ):
                r, g, b = img_input.getpixel((i, j))
                ...
                j < center_y
                and i
                <= center_x + ((center_y - j) * (img_output.size[0] // 2) // center_y)
                and i
                >= center_x - ((center_y - j) * (img_output.size[0] // 2) // center_y)
            ):
                r, g, b = img_input.getpixel((i, j))
...
def reverse_x_simbol_negative(img_input, coldepth):
    ...
    center_x = (
        img_output.size[0] // 2
    )  # posisi x tengah berada pada setengah dari lebar gambar
    center_y = (
        img_output.size[1] // 2
    )  # posisi y tengah berada pada setengah tinggi gambar
    ...
    for i in range(img_output.size[0]):
        for j in range(img_output.size[1]):
            if (
                i >= center_x
                and j
                <= center_y + ((i - center_x) * (img_output.size[1] // 2) // center_x)
                and j
                >= center_y - ((i - center_x) * (img_output.size[1] // 2) // center_x)
            ):
                r, g, b = img_input.getpixel((i, j))
                ...
                i < center_x
                and j
                <= center_y + ((center_x - i) * (img_output.size[1] // 2) // center_x)
                and j
                >= center_y - ((center_x - i) * (img_output.size[1] // 2) // center_x)
            ):
                r, g, b = img_input.getpixel((i, j))
...
def uts_1(img_input, coldepth, img_input2, coldepth2, alpha1, alpha2):
    ...
    center_y = (
        img_output.size[1] - img_negatived.size[1]
    ) // 2  # posisi y tengah berada pada setengah tinggi gambar
...
def WhiteTopHat(img_input, coldepth):
    ...
    img_opening = Opening(img_input, coldepth)
...
def BlackTopHat(img_input, coldepth):
    ...
    img_closing = Closing(img_input, coldepth)
